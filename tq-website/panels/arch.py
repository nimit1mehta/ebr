...
/* 3-up grid sized to the column, not to a fixed tile width - a fixed width
   overflowed the narrow category columns and pushed tiles into the next one */
.nv-logos{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}
.nv-logo{display:grid;place-items:center;width:100%;max-width:36px;aspect-ratio:1;
  background:var(--nv-d-node);border:1px solid rgba(255,255,255,.22)}
.nv-logo img{width:58%;height:58%;display:block;object-fit:contain}
/* no open-licensed mark available - same tile, brand initials */
.nv-logo-mono span{font-family:var(--nv-mono);font-size:11px;font-weight:700;
...
.nv-slot[data-slot="data"].is-open .nv-logos{max-height:130px;opacity:1}
